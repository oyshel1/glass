...
.hljs-tag {
            ...
            window.api.listenView.onSessionStateChanged((event, { isActive }) => {
                ...
                if (!wasActive && isActive) {
                    ...
                    this.updateComplete.then(() => {
                        ...
                        const liveQAView = this.shadowRoot.querySelector('live-qa-view');
                        if (sttView) sttView.resetTranscript();
                        if (summaryView) summaryView.resetAnalysis();
                        if (liveQAView) liveQAView.reset();
                    });
                    this.requestUpdate();
                }
                if (wasActive && !isActive) {
        ...
        this.updateComplete
            .then(() => {
                ...
                const activeContent = this.viewMode === 'transcript'
                    ...
                    : this.insightsMode === 'liveqa'
                        ? this.shadowRoot.querySelector('live-qa-view')
    ...
    toggleMic() {
        ...
        if (window.listenCapture?.setMicMuted) {
            window.listenCapture.setMicMuted(this.micMuted);
        }
        this.requestUpdate();
    }
    ...
    async handleCopy() {
        ...
        } else if (this.insightsMode === 'liveqa') {
            const liveQAView = this.shadowRoot.querySelector('live-qa-view');
            textToCopy = liveQAView ? liveQAView.getQAText() : '';
        } else {
            const summaryView = this.shadowRoot.querySelector('summary-view');
            ...
            <div class="assistant-container">
                <div class="top-bar">
                    ...
                    <div class="bar-controls">
                        ...
                        <button
                            class="mic-button ${this.micMuted ? 'muted' : ''}"
                            title="${this.micMuted ? 'Мікрофон вимкнено (тільки їх голос)' : 'Вимкнути мій мікрофон'}"
                ...
                ${this.viewMode === 'insights' ? html`
                    <div class="mode-toggle">
                        <button
                            class="mode-btn ${this.insightsMode === 'liveqa' ? 'active' : ''}"
                            @click=${() => { this.insightsMode = 'liveqa'; this.requestUpdate(); }}
                        >Live QA</button>
                        <button
                            class="mode-btn ${this.insightsMode === 'meeting' ? 'active' : ''}"
                            @click=${() => { this.insightsMode = 'meeting'; this.requestUpdate(); }}
                        >Meeting</button>
                    </div>
                ` : ''}
                ...
                <summary-view
                    .isVisible=${this.viewMode === 'insights' && this.insightsMode === 'meeting'}
